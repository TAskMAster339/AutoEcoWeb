...
export function useFeedback(status?: string, page = 0, limit = 20) {
  return useQuery({
    queryKey: userQueryKey('feedback', status, page, limit),
...
export function useMyFeedback() {
  return useQuery({ queryKey: userQueryKey('feedback', 'mine'), queryFn: api.fetchMyFeedback })
}
...
export function useCreateFeedback() {
  ...
  return useMutation({
    ...
    onSuccess: () => { void client.invalidateQueries({ queryKey: userQueryKey('feedback') }) },
  })
}
