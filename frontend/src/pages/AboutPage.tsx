...
function AutoTaggingGuide() {
    return (
        <>
            <Alert severity="info" sx={{ borderRadius: '8px' }}>
                Автотегирование — экспериментальная персональная функция. Она предлагает тег только тогда, когда уверена в результате; в остальных случаях операция сохраняется без тега и её можно разметить вручную.
            </Alert>
            <GuideSection title="Как включить и чему учится модель">
                <Text>Включить функцию можно в <Link component={RouterLink} to="/profile" color="primary" underline="hover" fontWeight={700}>профиле</Link>. Для обучения используются только ваши операции, где тег назначен вручную. Автоматический ответ не становится новым учебным примером сам по себе: сначала подтвердите его ручным сохранением или исправьте тег.</Text>
                <Text>Модель учитывает исходное и приведённое к единому виду название товара, вид операции и порядок величины суммы. Магазин используется лишь как осторожная подсказка. Поэтому покупка в продуктовом магазине не обязана стать «Едой»: по названию товара модель может отличить её от «Фастфуда».</Text>
            </GuideSection>
            <GuideSection title="Из каких частей она состоит">
                <Bullet><strong>Память повторяющихся товаров.</strong> Если одно и то же приведённое название встречалось не меньше трёх раз и минимум в 90 % случаев имело один тег, модель может сразу использовать эту закономерность.</Bullet>
                <Bullet><strong>Классификатор текста.</strong> Название разбивается на короткие фрагменты по 3–5 символов. Это помогает узнавать сокращения, окончания и неровные названия из чеков. Логистическая регрессия сравнивает эти признаки с вашими категориями.</Bullet>
                <Bullet><strong>Осторожная подсказка магазина.</strong> Она доступна только после 20 примеров с одинаковым магазином и совпадения тега минимум в 95 % случаев. Подсказка применяется, когда текст слаб, либо подтверждает тот же ответ.</Bullet>
                <Bullet><strong>Порог уверенности для каждого тега.</strong> Для разных категорий выбирается собственная граница. Не прошедший её ответ отбрасывается, а не показывается как достоверный.</Bullet>
            </GuideSection>
            <GuideSection title="Почему выбрана такая архитектура">
                <Text>У каждого пользователя свой словарь покупок и обычно сравнительно немного примеров. Символьные фрагменты устойчивы к сокращениям и опечаткам, не требуют общего словаря и занимают предсказуемый объём памяти. Логистическая регрессия быстро обучается на обычном процессоре и даёт оценку уверенности, которую можно проверить до включения автоматических ответов.</Text>
                <Text>Магазин намеренно не является главным признаком: простой вариант часто показывает высокую точность на обычных покупках, но не различает исключения внутри одного магазина. Его результат показан отдельно как «Только магазин», чтобы можно было сравнить полноценную модель с таким упрощением.</Text>
            </GuideSection>
            <GuideSection title="Как проверяется качество">
                <Text>Модель учится на более ранних операциях, а проверяется на последних 20 % — минимум на 20 записях. Такое разделение по времени ближе к реальному сценарию: прошлые покупки должны помочь распознать новые. Оно также раньше показывает, что привычки изменились.</Text>
                <Bullet><strong>Оценочная точность</strong> — доля правильных ответов среди тех, которые модель решилась принять. 97 % означает примерно 97 правильных тегов из 100 уверенных ответов на проверочной части, а не из всех ваших операций.</Bullet>
                <Bullet><strong>Покрытие</strong> — доля проверочных операций, для которых ответ оказался достаточно уверенным. Покрытие 68 % означает: примерно 68 из 100 операций получили бы тег, остальные 32 остались бы без него.</Bullet>
                <Bullet><strong>Обучающие операции</strong> — все доступные ручные примеры. Для запуска нужно минимум 50 операций и хотя бы два разных тега.</Bullet>
                <Bullet><strong>Поддерживаемые теги</strong> — категории, для которых удалось подтвердить безопасный порог. Нужно минимум пять учебных примеров, но этого недостаточно без успешной проверки на новых данных.</Bullet>
                <Bullet><strong>Только магазин</strong> — контрольный результат простого правила «у этого магазина обычно такой тег». Он помогает заметить, стала ли модель полезнее очевидной догадки по продавцу.</Bullet>
                <Alert severity="warning" sx={{ borderRadius: '8px' }}>Точность 100 % у отдельного тега не означает, что модель распознала все примеры. Она могла принять только несколько самых уверенных ответов. Поэтому у тега с 14 или 31 примером статус может отличаться: важны не только количество и точность, но и то, нашлось ли достаточно уверенных ответов на проверочной части.</Alert>
            </GuideSection>
            <GuideSection title="Когда модель обновляется">
                <Text>Новые ручные теги, исправления, импорт, переименование или удаление тегов и изменение правил создают новую версию данных. В профиле появляется состояние «Есть новые данные». Модель обновится перед следующим предсказанием или по кнопке «Обновить модель».</Text>
                <Text>Бинарный файл модели постоянно не хранится. В базе сохраняются версия данных, дата обучения и метрики, а готовая модель держится в оперативной памяти сервера для быстрых ответов. Сервер хранит до 32 недавно использованных моделей и при необходимости заново собирает нужную из операций пользователя. Поэтому пять пользователей получают пять независимых моделей, а не одну общую.</Text>
            </GuideSection>
            <GuideSection title="Как влияют правила и алиасы">
                <Text><WikiLink to="regex">Правила и алиасы</WikiLink> объединяют разные написания одного товара или магазина. Модель видит и исходное, и приведённое название: это сохраняет полезные детали и одновременно накапливает историю для одного объекта. После изменения алиаса модель помечается как устаревшая и проходит проверку заново.</Text>
                <Text>Не объединяйте алиасом разные по смыслу товары только ради красивого названия: модель потеряет различие между ними. Подробнее о создании категорий смотрите в разделе <WikiLink to="tags">«Теги и категории»</WikiLink>, а о переносе размеченной истории — в разделе <WikiLink to="data">«Импорт и экспорт»</WikiLink>.</Text>
            </GuideSection>
            <GuideSection title="Как помочь модели">
                <Bullet>Исправляйте неверный тег: следующая версия модели учтёт ручное решение.</Bullet>
                <Bullet>Используйте последовательные категории. Если «Услуги» означает слишком разные вещи, более конкретные теги могут обучаться устойчивее.</Bullet>
                <Bullet>Добавляйте алиасы только для настоящих вариантов одного названия.</Bullet>
                <Bullet>Оценивайте точность вместе с покрытием и статусами тегов, особенно после появления новой категории или изменения привычек.</Bullet>
            </GuideSection>
            <Text>Модель работает внутри AutoEco и не отправляет историю покупок внешнему сервису машинного обучения. Общие сведения о хранении аккаунта собраны в разделе <WikiLink to="storage">«Как хранятся данные»</WikiLink> и в <Link component={RouterLink} to="/privacy" color="primary" underline="hover" fontWeight={700}>политике конфиденциальности</Link>.</Text>
        </>
    )
}
