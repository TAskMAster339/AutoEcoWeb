    ...
    return (
        <>
            ...
            <GuideSection title="Сопоставляйте одинаковые товары">
                ...
                <Text>Для более честной аналитики уточняйте запрос: <Code>*биойогурт*200г*</Code> — пример поиска только упаковки 200 г. Используйте написание, которое реально встречается в позициях чека. В карточке «График цен товара» можно включить «Расширенный шаблон» и собрать условие тем же конструктором, что и на странице правил. Примеры шаблонов есть в разделе <WikiLink to="regex">«Правила и regex»</WikiLink>.</Text>
            </GuideSection>
    ...
    return (
        <>
            ...
            <GuideSection title="Теги и правила — разные инструменты">
                <Text>Тег отвечает на вопрос «к какой категории относится транзакция», а правило приводит разные названия товаров или магазинов к одному виду. Для устойчивого порядка используйте их вместе; настройка сопоставления описана в разделе <WikiLink to="regex">«Правила и regex»</WikiLink>.</Text>
            </GuideSection>
        </>
    )
}
...
const REGEX_ROWS = [
    ...
    ['*молоко*', 'Находит название, где до и после «молоко» может быть любой текст'],
    ['сыр ?00г', 'Знак ? заменяет ровно один любой символ'],
    ...
    return (
        <>
            ...
            <GuideSection title="Что такое regex">
                ...
                <Text>На странице «Правила» включите «Расширенный шаблон». Встроенный конструктор добавляет условия кнопками и сам собирает выражение: обычную строку, альтернативу «Или», цифры, пробелы, начало и конец названия.</Text>
            ...
            <GuideSection title="Готовые примеры">
                <Bullet><Code>*молоко*</Code> → «Молоко»: <Code>*</Code> означает любое количество любых символов.</Bullet>
                <Bullet><Code>сыр ?00г</Code> → «Сыр 200 г»: <Code>?</Code> означает ровно один любой символ.</Bullet>
                <Bullet><Code>молоко|кефир</Code> → «Молочные продукты»: вертикальная черта означает «или».</Bullet>
                <Bullet><Code>*\d+%*</Code> → «Процентный продукт»: <Code>\d+</Code> означает наличие одной или нескольких цифр.</Bullet>
