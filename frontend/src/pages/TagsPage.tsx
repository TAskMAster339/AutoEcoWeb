    ...
    return (
        <Stack spacing={2}>
            ...
            >
                <Stack spacing={2} onKeyDown={handleEditorKeyDown}>
                    ...
                    <Box sx={{ position: 'sticky', bottom: 0, zIndex: 1, pt: 1, pb: 'env(safe-area-inset-bottom)', bgcolor: 'background.paper' }}>
                        <Button fullWidth variant="contained" onClick={submit} disabled={createTag.isPending || updateTag.isPending} size="large">
            ...
            <ConfirmDialog
                ...
                title="Удалить тег?"
                message={deleteTarget ? `Тег «${deleteTarget.name}» будет удалён безвозвратно. У связанных транзакций тег будет очищен.` : ''}
                confirmLabel="Удалить"
                pending={deleteTag.isPending}
                error={deleteTag.isError ? (deleteTag.error instanceof Error ? deleteTag.error.message : 'Не удалось удалить тег') : null}
                onConfirm={() => {
