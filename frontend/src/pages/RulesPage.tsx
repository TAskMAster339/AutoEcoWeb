...
function RuleCard({ item, onEdit, onDelete }: { item: Alias; onEdit: (item: Alias) => void; onDelete: (id: string) => void }) {
  return (
    <Card sx={{ aspectRatio: '1 / 1', p: 2, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <Stack key={columnScope} spacing={1.5} sx={columnScope === 'product' ? { borderLeft: { md: '1px solid' }, borderColor: 'divider', pl: { md: 2 } } : undefined}>
              ...
              {items.length === 0 ? (
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 1.5 }}>
                  {items.map((item) => <RuleCard key={item.id} item={item} onEdit={openEdit} onDelete={(id) => void deleteAlias.mutate(id)} />)}
                </Box>
              )}
      ...
      <BottomSheet open={sheetOpen} onClose={closeSheet} title={editingAlias ? `Редактировать правило ${SCOPE_LABEL[scope]}` : `Новое правило ${SCOPE_LABEL[scope]}`}>
        <Stack spacing={2}>
          ...
          <Stack direction="row" spacing={1.5} sx={{ alignItems: 'center' }}>
            ...
            <TextField label="Приоритет" value={priority} onChange={(e) => setPriority(e.target.value.replace(/[^0-9]/g, ''))} inputMode="numeric" sx={{ width: 120 }} placeholder="0" slotProps={{ htmlInput: { maxLength: 4 } }} />
          </Stack>
