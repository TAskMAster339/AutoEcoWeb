...
function RowSelects({ user, disabled, onRoleChange, onStatusChange }: RowSelectsProps) {
    return (
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'minmax(0, 1fr) minmax(0, 1fr)', sm: '132px 150px' }, gap: 1, width: '100%' }}>
            <TextField select size="small" label="Роль" value={user.role} disabled={disabled} onChange={(e) => onRoleChange(e.target.value as UserRole)} sx={{ minWidth: 0 }}>
                {(Object.keys(ROLE_LABEL) as UserRole[]).map((value) => <MenuItem key={value} value={value}>{ROLE_LABEL[value]}</MenuItem>)}
            </TextField>
            <TextField
                select
                size="small"
                ...
                sx={{ minWidth: 0, '& .MuiInputBase-input': { color: STATUS_COLORS[user.status], fontWeight: 600 } }}
            >
                {(Object.keys(STATUS_LABEL) as UserStatus[]).map((value) => <MenuItem key={value} value={value}>{STATUS_LABEL[value]}</MenuItem>)}
            </TextField>
        </Box>
    )
}
        ...
        <Stack spacing={2} sx={isMobile ? undefined : { height: '100%', minHeight: 0 }}>
                ...
                <Stack spacing={1.5}>
                    <Stack direction="row" sx={{ alignItems: 'center', justifyContent: 'space-between' }}>
                        <Box>
                            ...
                            <Typography variant="caption" color="text.secondary">Роли, доступ и состояние аккаунтов</Typography>
                        </Box>
            ...
            {hasNextPage && (
                <Box sx={{ display: 'flex', justifyContent: 'center', py: 1 }}>
                    <Button fullWidth={isMobile} variant="outlined" onClick={() => void fetchNextPage()} disabled={isFetchingNextPage}>
