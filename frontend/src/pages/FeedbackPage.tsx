...
export function FeedbackAdminPanel() {
  ...
  return <Stack spacing={1.5}>
    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} justifyContent="space-between" alignItems={{ sm: 'center' }}>
      ...
      <Stack direction="row" spacing={1} justifyContent="flex-end" alignItems="center" sx={{ width: { xs: '100%', sm: 'auto' } }}>
    ...
    {(data?.items ?? []).map((item) => { const text = drafts[item.id] ?? ''; return <Card key={item.id} sx={{ p: 2 }}><Stack spacing={1.25}><Stack direction="row" justifyContent="space-between" gap={1}><Typography fontWeight={700}>{item.subject}</Typography><Typography variant="caption" color={item.status === 'open' ? 'warning.main' : 'success.main'}>{item.status === 'open' ? 'Нерешено' : item.status === 'answered' ? 'Отвечено' : 'Закрыто'}</Typography></Stack><Typography variant="caption" color="text.secondary">{item.email}</Typography><Typography variant="body2" sx={{ whiteSpace: 'pre-wrap' }}>{item.message}</Typography>{item.admin_reply && <Alert severity="success" sx={{ whiteSpace: 'pre-wrap' }}>Ответ: {item.admin_reply}</Alert>}{item.status === 'open' && <><Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap' }}>{FEEDBACK_TEMPLATES.map((template) => <Button key={template.label} size="small" variant="outlined" onClick={() => setDrafts((v) => ({ ...v, [item.id]: template.text }))}>{template.label}</Button>)}</Stack><TextField label="Ответ пользователю" value={text} onChange={(e) => setDrafts((v) => ({ ...v, [item.id]: e.target.value }))} multiline minRows={3} fullWidth /><Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}><Button variant="contained" disabled={!text.trim() || answer.isPending} onClick={() => answer.mutate({ id: item.id, data: { reply: text } })}>Ответить и закрыть</Button><Button variant="outlined" onClick={() => close.mutate(item.id)} disabled={close.isPending}>Закрыть без ответа</Button></Stack></>}</Stack></Card> })}
