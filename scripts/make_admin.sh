#!/usr/bin/env bash
#
# AutoEcoWeb — назначение роли администратора (role: user -> admin) по email.
#
# Скрипт НЕ создаёт пользователя: только обновляет роль существующей записи.
...
#   COMPOSE_FILE=docker-compose.dev.yml scripts/make_admin.sh user@example.com
...
echo "Назначаю администратором: $EMAIL"
...
UPDATE users SET role = 'admin' WHERE email = :'email' RETURNING email, role, status;
SQL
)" || {
...
echo "$OUTPUT"
echo "Готово: $EMAIL назначен администратором (role = admin)."
